...
let themeList = config.themeCss || ['/common/lib/theme-chalk/index.css', '/common/css/lib/default.css'];
const themes = config.themes || { ink: "#A9947F", lotus: "#b3b99f", cartoon: "#B0EDFF" };
let defaultColor = '#457CD6';
let profix = process.env.NODE_ENV === 'production' ? '' : location.origin;
let changeTheme = async (color, theme, resolve) => {
    let styleData = '';
    if (themes[theme]) {
        themeList.push(`/common/css/lib/${theme}.css`)
    }
    let apis = [];
    themeList.forEach((_s, s) => {
        apis.push(
            api('get', profix + _s, {
                noIntercept: true,
                cache: true
    ...
    styleData = styleData.replace(new RegExp("fonts/element-icons.woff", "g"), "/common/lib/theme-chalk/fonts/element-icons.woff");
    styleData = styleData.replace(new RegExp("fonts/element-icons.ttf", "g"), "/common/lib/theme-chalk/fonts/element-icons.ttf");
